...
static void change_version(struct wally_psbt* psbt, uint32_t version, const char* base64_in)
{
    ...
    if (ret != WALLY_OK)
        ret += fail("psbt set version to %u returned %d for %s", version, ret, base64_in);
}

/* Built-in tests shipped with Wally library */
int test_psbt_internal(void)
{
    size_t i;
    size_t ret = 0;
    ...
    for (i = 0; i < sizeof(invalid_psbts) / sizeof(invalid_psbts[0]); i++) {
        ...
        printf("Invalid Test %lu\n", i);

        if (wally_psbt_from_base64(invalid_psbts[i].base64, 0, &psbt) != WALLY_OK)
            continue;
        ret += fail("Should have failed to parse psbt %s", invalid_psbts[i].base64);
    }
    ...
    for (i = 0; i < sizeof(valid_psbts) / sizeof(valid_psbts[0]); i++) {
        ...
        if (wally_psbt_from_base64(base64_in, 0, &psbt) != WALLY_OK) {
            ret += fail("Failed to parse psbt %s", base64_in);
            continue;
        }

        if (wally_psbt_to_base64(psbt, 0, &output) != WALLY_OK) {
            ret += fail("Failed to base64 psbt %s", base64_in);
            continue;
        }

        if (valid_psbts[i].can_round_trip) {
            if (strcmp(output, base64_in) != 0) {
                ret += fail("psbt %s turned into %s?", base64_in, output);
                continue;
            }
        }
        ...
        if (wally_psbt_get_length(psbt, 0, &len) != WALLY_OK) {
            ret += fail("Failed to get psbt %s len", base64_in);
            continue;
        }

        bytes = malloc(len);
        if (wally_psbt_to_bytes(psbt, 0, bytes, len, &written) != WALLY_OK) {
            ret += fail("psbt %s could not to_bytes?", base64_in);
            continue;
        }

        if (len != written) {
            ret += fail("psbt %s to_bytes to %zu not %zu?", base64_in, written, len);
            continue;
        }

        if (wally_base64_from_bytes(bytes, len, 0, &output) != WALLY_OK) {
            ret += fail("Failed to convert psbt bytes to base64");
            continue;
        }

        if (valid_psbts[i].can_round_trip) {
            if (strcmp(output, base64_in) != 0) {
                ret += fail("psbt[%zi] base64 %s not %s", i, output, base64_in);
                continue;
            }
        }
        ...
        if (wally_psbt_from_base64(base64_in, 0, &psbt_clone) != WALLY_OK) {
            ret += fail("Failed to parse psbt clone %s", base64_in);
            continue;
        }

        if (wally_psbt_is_elements(psbt_clone, &is_elements) != WALLY_OK) {
            ret += fail("Failed to check PSET status %s", base64_in);
            continue;
        }
        ...
        if (!is_elements) {
            ...
            if (wally_psbt_combine(psbt_clone, psbt) != WALLY_OK) {
                ret += fail("Failed to combine psbts %s", base64_in);
                continue;
            }

            if (wally_psbt_to_base64(psbt_clone, 0, &output) != WALLY_OK) {
                ret += fail("Failed to base64 psbt combined %s", base64_in);
                continue;
            }

            if (valid_psbts[i].can_round_trip) {
                if (strcmp(output, base64_in) != 0) {
                    ret += fail("psbt combine %s turned into %s?", base64_in, output);
                    continue;
                }
            }
        }
        ...
        if (wally_psbt_clone_alloc(psbt, 0, &psbt_clone) != WALLY_OK) {
            ret += fail("Failed to clone psbts %s", base64_in);
            continue;
        }

        if (wally_psbt_to_base64(psbt_clone, 0, &output) != WALLY_OK) {
            ret += fail("Failed to base64 psbt clone %s", base64_in);
            continue;
        }

        if (valid_psbts[i].can_round_trip) {
            if (strcmp(output, base64_in) != 0) {
                ret += fail("psbt clone %s turned into %s?", base64_in, output);
                continue;
            }
        }
        ...
        if (is_elements) {
            ...
            ret += fail("Unknown psbt version %d in %s", psbt->version, base64_in);
            continue;
        }
    ...
    wally_cleanup(0);
    printf ("PSBT built-in tests done. Errors: %ld\n", ret);
    if (ret) 
        return WALLY_EINVAL;
    return WALLY_OK;
}
